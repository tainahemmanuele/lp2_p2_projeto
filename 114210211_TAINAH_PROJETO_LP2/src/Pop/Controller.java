...
/**
 * Classe criada com o objetivo de gerenciar todo o sistema do +Pop. A partir
 * das funcionalidades dessa classe que se pode cadastrar um usuario, criar um
 * post e administrar as informacoes acerca do sistema +Pop;
 * 
...
public class Controller implements Serializable {
	...
	/**
	 * Metodo utilizado para cadastrar um usuario no +Pop. Cria uma instancia de
	...
	public String cadastraUsuario(String nome, String email, String senha,
		...
		try {
			...
			Usuario usuario = new Usuario(verificacao.verificaNome(nome),
					...
					formataData.converteData(dataNascimento),
					verificacao.verificaImagem(imagem));
	...
	/**
	 * Metodo utilizado para cadastrar um usuario no +Pop. Cria uma instancia de
	 * objeto do tipo Usuario. Para isso, utiliza sobrecarga de metodo, ao
	 * utilizar como retorno a chamada do metodo de mesmo nome mas de assinatura
	 * distinta que se encontra no Controller.
	 * 
	 * @param nome
	 *            : nome do usuario;
	 * @param email
	 ...
	 * +Pop.Chama o metodo de mesmo nome que se encontra na classe Usuario.Essa
	 * chamada de metodo so e possivel se tiver um usuario logado no sistema.
	 * 
	 * @param atributo
	 *            : informacao que se deseja ver. Ex nome, email...
	 * @return: retorna a informacao pesquisada.
	 ...
	 * ou as adicione no +Pop.Chama o metodo de mesmo nome que se encontra na
	 * classe Usuario.Essa chamada de metodo so e possivel se tiver um usuario
	 * logado no sistema.
	 * 
	 * @param atributo
	 ...
	 * de mesmo nome que se encontra na classe Usuario.Essa chamada de metodo so
	 * e possivel se tiver um usuario logado no sistema.
	 * 
	 * @param atributo
	 ...
	 * que se encontra na classe Usuario.Essa chamada de metodo so e possivel se
	 * tiver um usuario logado no sistema.
	 * 
	 * @param mensagem
	 *            : mensagem do post;
	 * @param data
	 ...
	 * metodo de mesmo nome que se encontra na classe Usuario.Essa chamada de
	 * metodo so e possivel se tiver um usuario logado no sistema.
	 * 
	 * @param numeroPost
	 ...
	 * Usuario.Essa chamada de metodo so e possivel se tiver um usuario logado
	 * no sistema.
	 * 
	 * @param atributo
	 ...
	 * nome que se encontra na classe Usuario.Essa chamada de metodo so e
	 * possivel se tiver um usuario logado no sistema.
	 * 
	 * @param email
	 ...
	 * o metodo de mesmo nome que se encontra na classe Usuario.Essa chamada de
	 * metodo so e possivel se tiver um usuario logado no sistema.
	 * 
	 * @param email
	 *            : email do usuario a ser adicionado.
	 * @throws UsuarioException
	 *             : Excecao lancada caso deseje adicionar amizade de um usuario
	 ...
	 * amigos.Chama o metodo de mesmo nome que se encontra na classe
	 * Usuario.Essa chamada de metodo so e possivel se tiver um usuario logado
	 * no sistema.
	 * 
	 * @param email
	 ...
	 * nome que se encontra na classe Usuario.Essa chamada de metodo so e
	 * possivel se tiver um usuario logado no sistema. Chama o metodo
	 * adicionaHashtag() para adicionar as hashtags, caso existam, adicionadas
	 * ao post na lista de hashtags do Controller.
	 * 
	 * @param email
	 ...
	 * mesmo nome que se encontra na classe Usuario.Essa chamada de metodo so e
	 * possivel se tiver um usuario logado no sistema. Adiciona a hashtag recem
	 * adicionada no post a lista de hashtags do Controller.
	 * 
	 * @param email
	 *            : email do amigo a qual se deseja curtir o post;
	 * @param numeroPost
	 ...
	 * 
	 * @param email
	 *            : email do usuario que se deseja buscar.
	...
	/**
	 * Metodo utilizado para adicionar uma hashtag na lista de hashtags do
	 * Controller. Essas hashtags que sao adicionadas na lista refere-se as
	 * hashtags que sao colocadas nos post. O sistema +Pop armazena todas que
	 * sao utilizadas.
	 */
	...
	/**
	 * Metodo utilizado para adicionar uma hashtag na lista de hashtags do
	 * Controller. Essas hashtags que sao adicionadas na lista refre-se as
	 * hashtags que sao colocadas nos post. O sistema +Pop armazena todas que
	 * sao utilizadas.
	 * 
	 * @param tag
	 ...
	 * populares do +Pop, baseado na quantidade de pops de cada um.
	 * 
	...
	public String atualizaRankings() {
		...
		for (int i = 0; i < 3; i++) {
			if (usuarios.size() > i) {
				usuariosMenosPopulares += "(" + (i + 1) + ") "
						+ usuarios.get(i).getNome() + " "
						+ usuarios.get(i).getQuantidadePops() + "; ";
			}
		...
		for (int i = 0; i < 3; i++) {
			if (usuarios.size() > i) {
				usuariosMaisPopulares += "(" + (i + 1) + ") "
						+ usuarios.get(i).getNome() + " "
						+ usuarios.get(i).getQuantidadePops() + "; ";
			}

		}
		return usuariosMaisPopulares
				+ "| "
				+ usuariosMenosPopulares.substring(0,
						(usuariosMenosPopulares.length() - 1));
	 ...
	 * logado.Chama o metodo de mesmo nome que se encontra na classe Usuario.
	 * Essa chamada de metodo so e possivel se tiver um usuario logado no
	 * sistema.
	 * 
